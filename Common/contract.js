...
export function getTxContractEvents (web3, txReceipt, contractAddress, eventName, eventParams) {
  const events = []
  if (txReceipt.events) { for (const p in txReceipt.events) { events.push(txReceipt.events[p]) } }
  return events
    .filter(log => log.address.toLowerCase() === contractAddress.toLowerCase())
    .filter(log => log.raw?.topics[0] === web3.utils.soliditySha3(eventName + '(' + eventParams.map(param => param.type).join(',') + ')'))
    .map(log =>
